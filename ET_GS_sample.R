# Spatially balanced sampling setup for cropland GeoSurvey of Ethiopia
...
dir.create("ET_GS_sample", showWarnings=F)
setwd("./ET_GS_sample")

# download sampling grids
download("https://www.dropbox.com/s/fdweqhslofkxp5j/ET_sampling_grids.zip?raw=1", "ET_sampling_grids.zip", mode="wb")
unzip("ET_sampling_grids.zip", overwrite=T)
glist <- list.files(pattern="tif", full.names=T)
grids <- stack(glist)

# download GADM-L3 shapefile (courtesy: http://www.gadm.org)
download("https://www.dropbox.com/s/a11fm29a4qf8amt/ET_GADM3.zip?raw=1", "ET_GADM3.zip", mode="wb")
unzip("ET_GADM3.zip", overwrite=T)
shape <- shapefile("gadm36_ETH_3.shp")

# Sample setup ------------------------------------------------------------
# create a ROI image based on cropland mask
cpt <- 1
roi <- overlay(grids, fun=function(x) 
{return(ifelse(x[1] >= cpt, 1, 0))})
...
samp <- cbind(gadm[ ,c(5,7,9)], sloc) ## wrong needs to be fixed
colnames(samp) <- c("region", "zone", "woreda", "lon", "lat")
write.csv(samp, "ET_GS_sample.csv", row.names = F)
